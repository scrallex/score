...
- **HoVer multi-hop retrieval trial (staged):** Configure retrieval expansion to pull the top-2 secondary hops per claim via the HoVer adjacency graph, emit augmented manifests under `analysis/truth_packs/hover_dev_multi_hop/`, and fine-tune the reliability head for 2 epochs with the larger evidence memory (`scripts/train_reliability_attn.py --evidence-max 32 --attention-reg 1e-3`). Target output: `results/experiments/hover_multi_hop_trial.json` tracking admit precision/recall, attention entropy, and max-weight per head so E4 has concrete attention-specialisation evidence.
- **Structured sparsity sweep hook:** After the multi-hop checkpoint lands, run a local-window/rupture sweep (`local_window` ∈ {32, 64}, `rupture_tokens` ∈ {4, 8}) and persist metrics to `results/experiments/hover_multi_hop_sparsity.json` so E5 can cite throughput vs admit precision deltas alongside the retrieval trial.

## Immediate Actions
1. Prototype multi-hop retrieval or expanded evidence memory before the next HoVer adaptation run so attention entropy drops and admit probabilities move off the floor.
...
3. Integrate `scripts/evaluate_reliability.py` into CI to regenerate calibration plots and probability histograms automatically for FEVER, SciFact, and HoVer checkpoints.
